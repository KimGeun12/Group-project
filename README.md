...
블러 (blur_amount) 및 엣지 감지 임계값 (canny_threshold1 및 canny_threshold2)을 설정
...
```
그레이스케일 이미지에 블러 적용
Canny 엣지 검출을 사용하여 블러된 이미지에서 엣지를 식별
...
```
OpenCV의 findContours 함수를 사용하여 엣지가 감지된 이미지에서 등고선을 찾음
...
```
원본 이미지, 블러된 이미지 및 등고선이 그려진 이미지를 NumPy의 hstack 함수를 사용하여 결
